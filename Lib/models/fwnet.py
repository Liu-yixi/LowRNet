...
class FWConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True):
        super(FWConv2d, self).__init__()

        self.dn = FWBlock(
            in_channels, out_channels, stride=stride, kernel_size=kernel_size, padding=padding,
            square_noise=cfg['MODEL']['SQUARE_NOISE'],
            n_dict=cfg['MODEL']['EXPANSION_FACTOR'], 
            n_steps=cfg['MODEL']['NUM_LAYERS'], w_norm=cfg['MODEL']['WNORM'],
            yitaA=cfg['MODEL']['YITA_A'], tau=cfg['MODEL']['TAU'],initial_num=cfg['MODEL']['INITIAL_NUM']
        )
    ...
    def __init__(self, block, num_blocks, num_classes=10, Dataname=None):
        ...
        if 'cifar' in Dataname:
            ...
            self.layer0 = nn.Sequential(
                FWConv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(64),
                # nn.ReLU(inplace=True),
            )
        elif 'imagenet' in Dataname:
            self.layer0 = nn.Sequential(
                FWConv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False),
                nn.BatchNorm2d(64),
                # nn.ReLU(inplace=True),
...
if __name__ == '__main__':
    ...
    net = FWNet18(num_classes=100, cfg=cfg)
